...
for k=1:N
    for h=k+1:N
        % Hdip = a_{kh} (3Z - X - Y)
        Hdip=Hdip+0.5*a(k,h)*(2*mykron(speye(2^(k-1)),z,speye(2^(h-k-1)),z,speye(2^(N-h)))-... 
            mykron(speye(2^(k-1)),x,speye(2^(h-k-1)),x,speye(2^(N-h)))-...
            mykron(speye(2^(k-1)),y,speye(2^(h-k-1)),y,speye(2^(N-h)))) ; %added 0.5 .. ref - RL paper supplementary            
    end
end
